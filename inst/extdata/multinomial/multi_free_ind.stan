...
parameters{
  ...
  real<lower=0,upper=1> tauR; // the ratio of the slope of the exponential tail to the slope of the sigmoid center. For the right tail. Must be between 0 and 1. 
  real<lower=0,upper=width> deltaL; // same as deltaR, but for the left tail
  real<lower=0,upper=1> tauL; // same as tauR, but for the left tail
}
...
model{
  ...
  tauR ~ uniform(0,1); // prior for tauR
  deltaR ~ exponential(0.05); // prior for deltaR
  tauL ~ uniform(0,1); // prior for tauL
  deltaL ~ exponential(0.05); // prior for deltaL
